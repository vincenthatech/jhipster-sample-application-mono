  ...
  return (
    <div>
      <h2 id="job-heading" data-cy="JobHeading">
        <Translate contentKey="jhipsterSampleApplicationMonoApp.job.home.title">Jobs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplicationMonoApp.job.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/job/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplicationMonoApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </div>
      </h2>
        ...
        {jobList && jobList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplicationMonoApp.job.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('jobTitle')}>
                  <Translate contentKey="jhipsterSampleApplicationMonoApp.job.jobTitle">Job Title</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('jobTitle')} />
                </th>
                <th className="hand" onClick={sort('minSalary')}>
                  <Translate contentKey="jhipsterSampleApplicationMonoApp.job.minSalary">Min Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('minSalary')} />
                </th>
                <th className="hand" onClick={sort('maxSalary')}>
                  <Translate contentKey="jhipsterSampleApplicationMonoApp.job.maxSalary">Max Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('maxSalary')} />
                </th>
                <th>
                  <Translate contentKey="jhipsterSampleApplicationMonoApp.job.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationMonoApp.job.home.notFound">No Jobs found</Translate>
            </div>
          )
        )}
      </div>
